...
const common=rows.filter(x=>['DCO','DAD','DPO'].includes(x.codigo));const civil=rows.filter(x=>['DCI','DPC'].includes(x.codigo));const penal=rows.filter(x=>['DPP','DPE'].includes(x.codigo));
console.log(JSON.stringify({total:rows.length,common:common.length,civil:civil.length,penal:penal.length,byCode},null,2));
