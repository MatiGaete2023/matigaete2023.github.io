...
console.log(JSON.stringify({total:rows.length,common:rows.filter(x=>['DCO','DAD','DPO'].includes(x.codigo)).length,byCode},null,2));
if(rows.some(x=>['DCO','DAD','DPO'].includes(x.codigo))){console.error('El bloque común aún contiene riesgo alto.');process.exit(1);}
