...
const files=['blueprint.js','banco-comun.js','banco-civil.js','banco-penal.js','banco-familia.js','banco-laboral.js','revision-workflow.js','ajustes-l4.js','ajustes-l5.js','calidad-editorial.js'];
...
const common=rows.filter(x=>['DCO','DAD','DPO'].includes(x.codigo));const civil=rows.filter(x=>['DCI','DPC'].includes(x.codigo));const penal=rows.filter(x=>['DPP','DPE'].includes(x.codigo));const familia=rows.filter(x=>['DFA','DPFA'].includes(x.codigo));
console.log(JSON.stringify({total:rows.length,common:common.length,civil:civil.length,penal:penal.length,familia:familia.length,byCode},null,2));
