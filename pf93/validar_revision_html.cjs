...
const seen = new Set();
for (const id of ids) {
  if (seen.has(id)) errors.push(`ID HTML duplicado: ${id}`);
  seen.add(id);
}

const requiredIds = [
  'setup','track','mode','pool','topic','reviewState','search','start','session','question','options',
  'reviewBox','rvJuridico','rvFuente','rvRedaccion','rvDistractores','rvDificultad','rvDecision',
  'saveReview','applySuggestion','approveProduction','summary','funnel','coverage'
];
...
for (const src of localScripts) {
  ...
  expect(fs.existsSync(target), `Script local inexistente: ${src}`);
}
...
for (let i = 0; i < inline.length; i++) {
  try { new vm.Script(inline[i], { filename: `revision-inline-${i + 1}.js` }); }
  catch (err) { errors.push(`JS inline ${i + 1}: ${err.message}`); }
}
